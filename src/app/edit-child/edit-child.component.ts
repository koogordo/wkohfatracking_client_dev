...
export class EditChildComponent implements OnInit {
  ...
  updateClients() {
    ...
    var clientFNameIndex = this.formGroupService.indexQuestionGroup(this.clientFormGroup.getRawValue(), 'Child FName');
    var clientLNameIndex = this.formGroupService.indexQuestionGroup(this.clientFormGroup.getRawValue(), 'Child LName');
    var ssnIndex = this.formGroupService.indexQuestionGroup(this.clientFormGroup.getRawValue(), 'Child SSN');
    ...
    this.databaseService.getFamily(this.familyID).then((family) => {
      ...
      family.child[this.parsedClientID[2]].clientFName = clientFName.input;
      family.child[this.parsedClientID[2]].clientLName = clientLName.input;
      family.child[this.parsedClientID[2]].ssn = ssn.input;
      family.child[this.parsedClientID[2]].form = this.clientFormGroup.getRawValue();
